        ...
        case 2:
        {
          ...
          if (i_o < i_n)
          {
            ...
            if(k_o < k_n)
            {
              ...
              if (z1 < k_n * dz)
              {
                simple_current_distribution(r1, z1,
                                            P_POS_OLD_R((**i)), P_POS_OLD_Z((**i)),
                                            i_n-1, k_n-1,
                                            P_CHARGE((**i)));
                simple_current_distribution(r2, z2,
                                            r1, z1,
                                            i_n, k_n-1,
                                            P_CHARGE((**i)));
                simple_current_distribution(P_POS_R((**i)), P_POS_Z((**i)),
...
void Current::azimuthal_current_distribution()
{
  ...
  for (auto ps = species_p.begin(); ps != species_p.end(); ++ps)
    for (auto i = (**ps).particles.begin(); i != (**ps).particles.end(); ++i)
    {
      ...
      int r_i_shift = 0;  // number of particle i cell
      int z_k_shift = 0;  // number of particle k cell
  ...
  {
    MSG_FIXME("simple_current_distribution: r_i_shift is more, than current[0].size_x() - 2. Workaround applying");
    r_i_shift = current[0].size_x() - 2;
  }

  if (z_k_shift > current[0].size_y() - 2)
  {
    MSG_FIXME("simple_current_distribution: z_k_shift is more, than current[0].size_x - 2 . Workaround applying");
      ...
      if(P_POS_R((**i)) > dr)
      {
        ...
        value = rho * P_VEL_PHI((**i));
        current[1].inc(r_i_shift, z_k_shift, value);

        // weighting in j[i + 1][k] cell
        rho = ro_v * CYL_RNG_VOL(dz1, r2, r3) / v_2;
        value = rho * P_VEL_PHI((**i));
        current[1].inc(r_i_shift + 1,z_k_shift, value);

        // weighting in j[i][k + 1] cell
        rho = ro_v * CYL_RNG_VOL(dz2, r1, r2) / v_1;
        value = rho * P_VEL_PHI((**i));
        current[1].inc(r_i_shift, z_k_shift + 1, value);

        // weighting in j[i + 1][k + 1] cell
        rho = ro_v * CYL_RNG_VOL(dz2, r2, r3) / v_2;
        value = rho * P_VEL_PHI((**i));
        current[1].inc(r_i_shift + 1, z_k_shift + 1, value);
