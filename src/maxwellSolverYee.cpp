  ...
  : MaxwellSolver(_geometry, _time, _species_p, _current)
{
  ...
  field_h_at_et = Grid3D<double> (geometry->r_grid_amount, geometry->z_grid_amount, 2);
  field_h_at_et = 0.;
  ...
  for (int i = 0; i < geometry->r_grid_amount; i++)
    for (int k = 0; k < geometry->z_grid_amount; k++)
    {
      ...
      double alpha_t_z = time->step
        * (
          (field_e(1, i+1, k) + field_e(1, i, k)) / (2. * dr * (i + 0.5 + geometry->bottom_r_grid_number))
  ...
  for (unsigned int i = r_begin; i < r_end; i++)
    for (unsigned int k = z_begin; k < z_end; k++)
    {
      ...
      field_e[2].dec(i, k, (curr(2, i, k)
                            ...
                            / (2. * dr * (i + geometry->bottom_r_grid_number))) * koef_h);
