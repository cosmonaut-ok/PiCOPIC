...
def run(config_path, specie, clim, video_file=None, time_range=None, cmap=None, frame_step=1, frame_size=None, dry_run=False, view=False, use_grid=False, plot_image_interpolation='bicubic'):
    ...
    video_file = os.path.join(config_path, 'density_movie.avi') if not video_file else video_file
    ...
    metadata = dict(title='Plasma Density Evolution Movie', artist='Matplotlib',
    ...
    with writer.saving(fig, video_file, reader.meta.figure_dpi):
        for i in range(start_frame, end_frame):
            if i % frame_step == 0:
                ...
                sys.stdout.flush()
                data = reader.rec("density/{}".format(specie), frame_src_size, i)[frame_size[0]:frame_size[2], frame_size[1]:frame_size[3]]
